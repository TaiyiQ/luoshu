# ADR 0001: Geometry-free, order-explicit IR between circuit and schedule
...
The compiler currently translates OpenQASM 3 circuits end-to-end into a
hardware schedule (per-atom `load`/`move`/`store`/`rydberg`/`raman` ops).
All routing and placement decisions — which qubits move, in what order,
where they rest — are made internally by the compiler passes.

At the current maturity of neutral-atom compilation, human scheduling
insight often beats heuristics, but the only ways to inject it today are
(a) rewriting the input circuit or (b) patching compiler internals.
There is no stable interface in between.

The literature offers no representation at the altitude we need:

- **ZAIR** (ZAC, HPCA'25, [arXiv:2411.11784](https://arxiv.org/abs/2411.11784))
  is a zoned-architecture IR, but it is compiler-internal and site-explicit —
  qubits are located by `(qubit, array, row, col)` tuples. ZAC also infers
  qubit *reuse* (keeping atoms in the entanglement zone) automatically.
- The **MQT abstract model** ([arXiv:2405.08068](https://arxiv.org/abs/2405.08068))
  formalizes load/store/move at the schedule level, for automated routing,
  not for human authorship.
- **Bloqade shuttle/lanes** (QuEra, on Kirin) lets users author movement,
  but at trap-position/trajectory level — below our target altitude.
- End-to-end compilers (ZAP, Enola, Atomique, PowerMove, DasAtom) expose
  no human-facing layer at all.

The gap: a representation that is *user-authorable*, *geometry-free*, and
*order-explicit* — the user owns the execution strategy, the compiler owns
the physical realization.

## Decision

Introduce an IR between the logical (circuit) stage and the physical
(schedule) stage with the following design.
...
|---|---|---|
| `reservoir` | unordered pool (layout owned by upstream rearrangement / `assembly.json`) | across program |
| `computing` row | totally ordered sequence | across runs |
| live AOD row | totally ordered sequence | between `pick` and its `drop`s |

The two orders plus the pool are the *entire* logical machine state. No
coordinates, site indices, or trajectories appear in the IR.
...
```
row   q…                     ; declaration/assertion of the computing-row order
pick  q… from <zone>         ; zone → AOD row; list order = left-to-right row order
cz    [mover,target]…        ; one movement step + one global Rydberg pulse
drop  q… -> reservoir        ; AOD → reservoir (no anchor needed)
drop  q  -> computing after q' ; AOD → computing row, anchored insertion
```

- **`row`** at program start declares the initial computing-zone occupancy
...
### Legality (checkable at the IR level, compiler-independent)

Per `cz` line: walking the movers in pick order, their targets must appear
in computing-row order, with room between neighbors' destinations for any
resting movers. Per `drop … -> computing`: anchor order must match mover
order. Violations are logical errors diagnosable against the IR alone,
before any physics.
...
- Instruction verbs are **`pick`/`drop`**, *not* `load`/`store`: the
  schedule output already uses `"op": "load"`/`"op": "store"` (see
  `src/serialize.zig`, `src/schedule.zig`), the mapping is not 1:1 (one
  `pick` of n qubits expands to n schedule `load`s plus moves), and the
  MQT literature has established load/store as schedule-level vocabulary.
  Sharing verbs across adjacent layers invites exactly the level
  confusion the IR exists to prevent.
- Zones are named `reservoir` and `computing`.
- Logical pairs use **brackets** `[1,0]`; parentheses stay reserved for
  physical coordinates `(x, y)` in schedule/trace output.

### Example

```
row    0 8 2 6 5 4              ; initial computing-zone order
pick   1 3 9 7 from reservoir
cz     [1,0] [3,8] [9,6] [7,5]
cz     [1,2] [3,5] [9,4]        ; 7 rests
cz     [1,5] [7,4]              ; 3, 9 rest
drop   9 7 -> reservoir
drop   1 -> computing after 0   ; kept for the next run
drop   3 -> computing after 2

row    0 1 8 2 3 6 5 4          ; checkpoint
pick   6 5 from computing       ; last run's targets become movers
cz     [6,1] [5,3]
drop   6 5 -> reservoir
```
